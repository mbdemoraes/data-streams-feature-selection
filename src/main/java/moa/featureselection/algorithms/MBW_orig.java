...
*/
public class MBW_orig implements Classifier, UpdateableClassifier {
  ...
  public void buildClassifier(Instances insts) throws Exception {
    ...
    Enumeration<Attribute> enum2 = insts.enumerateAttributes();
    while (enum2.hasMoreElements()) {
      Attribute attr = (Attribute) enum2.nextElement();
      if (!attr.isNominal()) {
  ...
  public static void main(String[] argv) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new MBW_orig(), argv));
    } catch (Exception e) {
